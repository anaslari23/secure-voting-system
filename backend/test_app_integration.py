import unittest
import json
import os
import sys
# Add backend to path
sys.path.append(os.path.join(os.path.dirname(__file__), '../backend'))

import shutil
from app import app
from src.bulletin_board import BulletinBoard

class SecureVotingWebTest(unittest.TestCase):
    
    def setUp(self):
        # Configure app for testing
        app.config['TESTING'] = True
        app.secret_key = 'test_secret'
        self.client = app.test_client()
        
        # Reset Bulletin Board (SQLite)
        if os.path.exists("secure_voting.db"):
            os.remove("secure_voting.db")
        # Re-init DB
        from src.db import init_db
        init_db()
        
        self.bb = BulletinBoard()
        
        # Reset Voter State (Seed new DB)
        # Note: init_db already seeds mock voters with has_voted=False
        
        # Ensure Keys exist (generate if not)
        if not os.path.exists("keys/public_key.json"):
            from src.keygen import generate_keypair
            generate_keypair(key_size=512) # Fast keys for test

    def test_full_voting_flow(self):
         print("\n--- TEST: Web Kiosk Full Flow ---")
         
         # 1. Access Home Page
         resp = self.client.get('/')
         self.assertEqual(resp.status_code, 200)
         print("[x] Home Page Loaded")
         
         # 2. Login with Valid ID (Step 1: ID Check)
         voter_id = "1111-2222-3333"
         resp = self.client.post('/login', json={'aadhaar': voter_id})
         data = json.loads(resp.data)
         self.assertEqual(data['status'], 'otp_sent')
         print(f"[x] ID Verified. OTP Sent.")
         
         # 3. Complete Login (Step 2: OTP Check)
         # Extract Mock OTP from session (since we can't see server logs in test)
         with self.client.session_transaction() as sess:
             otp = sess['otp']
             
         resp = self.client.post('/verify_otp', json={'otp': otp})
         data = json.loads(resp.data)
         self.assertEqual(data['status'], 'success')
         
         # Check redirection works manually or access vote page
         resp = self.client.get('/vote')
         self.assertIn(b'DIGITAL BALLOT UNIT', resp.data)
         print(f"[x] Logged in as {voter_id} via 2FA")
         
         # 3. Submit Vote (Yes = 1)
         resp = self.client.post('/submit_vote', data={'vote': '1'})
         data = json.loads(resp.data)
         self.assertEqual(data['status'], 'success')
         print("[x] Vote Submitted via API")
         
         # 4. Verify Ledger Update
         ledger = self.bb.get_all_ballots()
         self.assertEqual(len(ledger), 1)
         last_ballot = ledger[0]['ballot']
         print(f"[x] Ledger Verified: Block #{ledger[0]['index']} Mined")
         
         # 5. Verify Receipt Page
         with self.client.session_transaction() as sess:
             # Manually simulate session persistence for next request if needed, 
             # but client cookies handle it.
             pass
             
         resp = self.client.get('/success')
         self.assertIn(b'VOTE RECORDED', resp.data)
         # Verify Tracking Hash (ciphertext snippet) is present
         short_hash = last_ballot['ciphertext'][:20]
         self.assertIn(short_hash.encode(), resp.data)
         print(f"[x] Receipt Page Generated with Hash: {short_hash}...")
         
         # 6. Verify Anti-Double Voting
         # 6. Verify Anti-Double Voting
         resp = self.client.post('/login', json={'aadhaar': voter_id})
         data = json.loads(resp.data)
         self.assertIn('already voted', data.get('error', '').lower())
         print("[x] Double Voting Blocked")

if __name__ == '__main__':
    unittest.main()
